...
Output: una cartella ./campioni_cv/ con un PDF per ogni candidato generato casualmente.
"""
import os
import random
...
# ---------------------------------------------------------------------------
# Generazione avatar placeholder
# ---------------------------------------------------------------------------
...
def get_styles():
    return {
        "name": ParagraphStyle("name", fontName="Helvetica-Bold", fontSize=20, textColor=BLUE_DARK, leading=24, spaceAfter=2),
        "role": ParagraphStyle("role", fontName="Helvetica", fontSize=11, textColor=BLUE_MID, leading=14, spaceAfter=4),
        "contact": ParagraphStyle("contact", fontName="Helvetica", fontSize=8, textColor=GREY_TEXT, leading=12),
        "profilo": ParagraphStyle("profilo", fontName="Helvetica-Oblique", fontSize=9, textColor=GREY_TEXT, leading=13, spaceAfter=4),
        "entry_title": ParagraphStyle("entry_title", fontName="Helvetica-Bold", fontSize=9, textColor=BLACK, leading=12),
        "entry_sub": ParagraphStyle("entry_sub", fontName="Helvetica", fontSize=8.5, textColor=BLUE_MID, leading=11),
        "entry_date": ParagraphStyle("entry_date", fontName="Helvetica", fontSize=8, textColor=colors.HexColor("#777777"), leading=11, alignment=TA_RIGHT),
        "entry_body": ParagraphStyle("entry_body", fontName="Helvetica", fontSize=8.5, textColor=GREY_TEXT, leading=12, spaceAfter=4),
        "skill_label": ParagraphStyle("skill_label", fontName="Helvetica-Bold", fontSize=8.5, textColor=BLACK, leading=11),
        "skill_item": ParagraphStyle("skill_item", fontName="Helvetica", fontSize=8.5, textColor=GREY_TEXT, leading=11),
        "lang_name": ParagraphStyle("lang_name", fontName="Helvetica-Bold", fontSize=8.5, textColor=BLACK),
        "lang_level": ParagraphStyle("lang_level", fontName="Helvetica", fontSize=8.5, textColor=GREY_TEXT),
        "cert": ParagraphStyle("cert", fontName="Helvetica", fontSize=8.5, textColor=GREY_TEXT, leading=12, leftIndent=5),
        "footer": ParagraphStyle("footer", fontName="Helvetica-Oblique", fontSize=7, textColor=GREY_LINE, alignment=TA_CENTER),
    }
...
def build_cv(candidato: dict, output_path: str):
    ...
    header_data = [[Paragraph(candidato["nome"], S["name"]), AvatarImage(avatar_buf, avatar_size)]]
    header_table = Table(header_data, colWidths=[usable_w - avatar_size - 4 * mm, avatar_size])
    ...
    info_rows = [
        ...
        [
            Paragraph(f"✉ {candidato['email']}   ☎ {candidato['telefono']}", S["contact"]),
            Paragraph(f"🏠 {candidato['indirizzo']}", S["contact"]),
        ],
        [
            Paragraph(f"🔗 {candidato['linkedin']}", S["contact"]),
            Paragraph(f"Nato/a il {candidato['data_nascita']} a {candidato['luogo_nascita']}   |   Nazionalità: {candidato['nazionalita']}", S["contact"]),
        ],
    ]
    ...
    for edu in candidato["istruzione"]:
        ...
        if voto_str:
            desc += f"   |   {voto_str}"
        row = [
            [Paragraph(edu["titolo"], S["entry_title"]), Paragraph(desc, S["entry_sub"])],
            [Paragraph(date_str, S["entry_date"])],
        ]
        t = Table([row], colWidths=[usable_w * 0.72, usable_w * 0.28])
        t.setStyle(TableStyle([
    ...
    for categoria, items in candidato["competenze"].items():
        skill_rows.append([Paragraph(categoria, S["skill_label"]), Paragraph("  ·  ".join(items), S["skill_item"])])
    if skill_rows:
    ...
    story.append(Spacer(1, 2 * mm))
    lang_rows = [[
        Paragraph("Lingua", S["skill_label"]), Paragraph("Livello", S["skill_label"]),
        Paragraph("Comprensione", S["skill_label"]), Paragraph("Parlato", S["skill_label"]), Paragraph("Scritto", S["skill_label"]),
    ]]
    for lingua, livello in candidato["lingue"]:
        if "Madrelingua" in livello:
            ...
            bars = [2, 2, 2]
        lang_rows.append([
            Paragraph(lingua, S["lang_name"]), Paragraph(livello, S["lang_level"]),
            SkillBar(bars[0]), SkillBar(bars[1]), SkillBar(bars[2]),
        ])
    col_w = usable_w / 5
    ...
    story.append(Paragraph(
        "Autorizzo il trattamento dei dati personali ai sensi dell'Art. 13 del D.Lgs. 196/2003 e del Regolamento UE 2016/679 (GDPR).",
        S["footer"],
    ))

    doc = SimpleDocTemplate(
        output_path, pagesize=A4, leftMargin=MARGIN_L, rightMargin=MARGIN_R,
        topMargin=MARGIN_T, bottomMargin=MARGIN_B,
        title=f"Curriculum Vitae — {candidato['nome']}",
        author=candidato["nome"], subject="Curriculum Vitae Europass", creator="FluxHR CV Generator",
    )
    ...
    def add_page_number(canvas_obj, doc_obj):
        ...
        canvas_obj.drawRightString(W - MARGIN_R, MARGIN_B / 2, f"Curriculum Vitae di {candidato['nome']} — Pagina {doc_obj.page}")
